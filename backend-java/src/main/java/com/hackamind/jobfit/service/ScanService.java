...
public class ScanService {
    ...
    public List<AnalyzeResponse> history(String sessionId, Long userId) {
        ...
        if (userId != null) {
            results = scanResultRepository.findByResumeUserIdOrderByCreatedAtDesc(userId);
        } else {
            results = scanResultRepository.findByResumeSessionSessionIdOrderByCreatedAtDesc(sessionId);
        }
